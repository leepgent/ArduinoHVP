...
static const uint8_t LV_MODE_LED = 6;
...
static bool reset_is_inverted = 0;

//#define reset_target(b) digitalWrite(RESET, b ? HIGH : LOW)
inline void reset_target(bool b) {
  if (reset_is_inverted) b = !b;
    digitalWrite(RESET, b ? HIGH : LOW);
}
static void avrisp();
...
void setup_lv() {
  ...
  pinMode(LV_MODE_LED, OUTPUT);
  digitalWrite(LV_MODE_LED, HIGH);
  pinMode(VCC, OUTPUT); // power on tgt mcu
...
void heartbeat() {
  ...
  if (hbval > 192) {
    hbdelta = -hbdelta;
    // if invert mode active, also flash the LVMODE LED
  if (reset_is_inverted) {
    digitalWrite(LV_MODE_LED, HIGH);
  }
  }
  if (hbval < 32) {
...
void reset_target(bool reset) {
  // To reset, drive target_reset pin HIGH
  // this opens the switch and drains off the VCC/12V to Gnd
  // driving collector to 0V
  // AVR is reset-low so this resets it
  digitalWrite(RESET, reset ? HIGH : LOW);
  // low opens the transistor and drives collector to 
...
void loop_lv(void) {
  ...
  // If, from here on out, the voltage-mode selector pin is jumpered to ground again,
  // we will switch to (and latch on) inverted-mode RESET output to overcome a bug in the PCB design.
  // We will make 'invert mode' mean 'target the Attiny socket' since if we accidentally leave the jumper on and switch to HV mode - it won't kill an attiny
  // the attiny socket reset pin goes through an inverting transistor so reset pin high -> drives target reset pin low -> actually hold the device in reset

  if (!reset_is_inverted) {
    if (digitalRead(SELECT_HV_MODE_PIN) == LOW) {
      reset_is_inverted = 1;
    }
  }
